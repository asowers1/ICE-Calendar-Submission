...
class SavedViewController: UITableViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UISearchDisplayDelegate {
    ...
    func setupPath() -> String {
        ...
        if(!fileManager.fileExistsAtPath(path)){
            ...
            if let bundlePath = NSBundle.mainBundle().pathForResource("savedEventsList", ofType: "plist"){
                let resultDictionary = NSMutableDictionary(contentsOfFile: bundlePath)
                println("Bundle savedEventsList.plist file is --> \(resultDictionary?.description)")
                fileManager.copyItemAtPath(bundlePath, toPath: path, error: nil)
            } else {
                println("savedEventsList.plist not found. Please, make sure it is part of the bundle.")
            }
        }else {
            ...
            if let dictionary = myDictionary {
                if dictionary.count == 0 {
                    ...
                    for (myKey,myValue) in dictionary {
                        let array:NSMutableArray = [myValue]
                        self.categories.append(array)
                    }
                }
            } else {
